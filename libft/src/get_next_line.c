...
/*   Updated: 2016/03/30 06:30:16 by bkabbas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
						...
						char **line)
{
	...
	{
		buffer[read_ret] = '\0';
		new = ft_memjoin(file->buffer, buffer, file->stock, read_ret + 1);
		free(file->buffer);
		file->buffer = new;
		file->stock += read_ret;
	...
	if (*end != '\n')
	{
		list_remove_value_inner(reminder, file, list_default_remove_functor);
		return (0);
	}
...
int				get_next_line(int const fd, char **line)
{
	t_filebuffer	*file;
	char			*end;
	static t_list	reminder = { NULL, NULL, NULL, 0, 0, 8 };
